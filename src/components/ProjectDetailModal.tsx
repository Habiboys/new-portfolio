...
import { useProjectDetails } from "@/hooks/usePortfolio";
...
export default function ProjectDetailModal({ project, onClose }: Props) {
  ...
  const needsDetails = Boolean(project && project.images.length === 0);
  const { data: fullProject, isLoading } = useProjectDetails(
    needsDetails ? project?.id : null
  );
  ...
  if (!project || !displayProject) return null;

  const images =
    displayProject.images?.length > 0
      ? displayProject.images
      : [displayProject.previewImage];
          ...
          >
            ...
            <div className="relative w-full aspect-video bg-gray-100 overflow-hidden">
              ...
              {totalImages > 1 && (
                <>
                  ...
                  <button
                    onClick={() => setImgIndex((prev) => (prev + 1) % totalImages)}
            ...
            <div className="p-6 md:p-10 space-y-8">
              ...
              {displayProject.features?.length > 0 && (
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {displayProject.features.map((f, i) => (
              ...
              <div className="flex flex-wrap gap-3 pt-4 border-t border-gray-200">
                ...
                {displayProject.sourceCode && (
                  <a
                    href={displayProject.sourceCode}
                    target="_blank"
                    rel="noopener noreferrer"
